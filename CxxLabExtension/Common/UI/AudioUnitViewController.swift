...
private let log = Logger(subsystem: "com.nikolozi.CxxLabExtension", category: "AudioUnitViewController")

public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<CxxLabExtensionMainView>?
	...
	*/
    ...
    public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
        audioUnit = try CxxLabAudioUnit(componentDescription: componentDescription, options: [])

        guard let audioUnit = self.audioUnit as? CxxLabAudioUnit else {
            log.error("Unable to create CxxLabAudioUnit")
            return audioUnit!
        }
        ...
        audioUnit.setupParameterTree(CxxLabExtensionParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = CxxLabExtensionMainView(parameterTree: observableParameterTree)
